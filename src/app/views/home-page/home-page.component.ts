...
export class HomePageComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ngZone.runOutsideAngular(
      () => {
        this.checkProfile = interval(500).subscribe(
          (interval: number) => {
                ...
                () => {
                  ...
                  if (!this.wasShown) {
                    ...
                    this.notificationService.showToastNotification(new NotificationItem(null,
                      NotificationMessage.CreateCommonMessage(NotificationType.Info, 'Profile', this.title)),
                      false);
                  }
                }
              );
                ...
                this.auth.askAboutProfile(this.auth.user.profile).subscribe(
                  ...
                  (error) => {
                    ...
                    this.ngZone.run(
                      () => {
                        this.notificationService.showToastNotification(new NotificationItem(null,
                          NotificationMessage.CreateCommonMessage(NotificationType.Error, 'Profile error',
  ...
  getBuildData() {
    this.http.get(environment.apiUrl + '/version').subscribe(
      (x: any) => {
        const data = x.build;
