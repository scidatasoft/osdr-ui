...
export class NodesApiService {
  ...
  setCurrentNode(id?: string): Observable<any> {
    ...
    request = { Id: id };
    return this.http.post(environment.apiUrl + '/nodes/current', request, { observe: 'response' }).pipe(map(
      (x) => {
        return {
  ...
  getBreadCrumbs(id: string = null): Observable<{ Id: string, Name: string }[]> {
    ...
    return this.http.head(environment.apiUrl + `/nodes/${folder_id}`, { observe: 'response', responseType: 'text' }).pipe(map(
      (x) => {
  ...
  setCurrentNodeWithoutBreadCrumbs(id?: string): Observable<BrowserDataItem> {
    let request: { Id?: string };
    request = { Id: id };
    return this.http.post<BrowserDataItem>(environment.apiUrl + '/nodes/current', request).pipe(map(
      (x) => {
  ...
  getNodeWithFilter(params): Observable<any> {
    const paramsUrl = params ? this.joinParams(params) : '';
    return this.http.get(environment.apiUrl + `/nodes${paramsUrl}`, { observe: 'response' }).pipe(map(
      x => (
        {
  ...
  getSharedFiles(): Observable<any> {
    return this.http.get(environment.apiUrl + `/nodes/?$filter=accessPermissions.isPublic eq true`, { observe: 'response' }).pipe(map(
      x => (
        {
          data: x.body as BrowserDataItem[],
          page: JSON.parse(x.headers.get('x-pagination')),
  ...
  getPublicFiles(params): Observable<any> {
    ...
    return this.http.get(environment.apiUrl + `/nodes/public${paramsUrl}`, { observe: 'response' }).pipe(map(
      x => (
        {
          data: x.body as BrowserDataItem[],
          page: JSON.parse(x.headers.get('x-pagination'))
  ...
  getRecordName(parentId: string): Observable<string> {
    return this.http.get<{ id: string, name: string }>(environment.apiUrl + `/nodes/${parentId}?$projection=name`).pipe(map(
      (data) => {
        return data.name;
      }
    ));
